...
object Huffman {
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def get (tree: CodeTree)(c : Char): List[Bit] = tree match {
        case Leaf(_, _) => List()
        case Fork(left, right, _, _) => if (chars(left).contains(c)) 0::get(left)(c) else 1::get(right)(c)
      }

      text.flatMap(get(tree))
    }
